...
'''
    ...
    stage('Build') {
      steps {
        sh '''#!/bin/bash
docker build -t 226347592148.dkr.ecr.ap-northeast-1.amazonaws.com/demo-frontend:v$VERSION . 
'''
      }
    }
    ...
    stage('Post-Build') {
      steps {
        sh 'echo $VERSION'
        sh 'docker push 226347592148.dkr.ecr.ap-northeast-1.amazonaws.com/demo-frontend:v$VERSION'
        sh '''#!/bin/bash
rm -rf *
rm -rf .*'''
      }
...
sed -i "s|tag:.*|tag: v$VERSION|g" frontend-skills-repo/values.yaml
